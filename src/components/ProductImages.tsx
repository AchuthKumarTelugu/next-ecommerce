...
type imagelistModal={
  ...
  // ]
  const [imageUrl, setImageUrl] = useState(images[0].image?.url)
  ...
  <div className="mt-14 flex gap-4 gap-y-4 flex-wrap lg:flex-nowrap justify-between ">
   
    {
      images.map((value,index)=>(
        <div className="relative h-32 w-1/4 cursor-pointer" key={value.id} onClick={()=>ChangeImageUrl(value.image?.url)} >
    <Image src={value.image?.url} fill sizes="30vw" className='object-cover '/>
    </div>
      ))
    }
  </div>
